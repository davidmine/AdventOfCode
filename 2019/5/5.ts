...
function getOpCodeExecutionFunction(opCode : number) {
  ...
  return {
    ...
    1: (params : CommandParams) : State => {
      ...
      var firstValue = first.mode == Mode.Position ? params.state.puzzleInput[first.value].toNum() : first.value;
      var secondValue = second.mode == Mode.Position ? params.state.puzzleInput[second.value].toNum() : second.value;
